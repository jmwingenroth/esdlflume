%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function Vectrino_processing_lowSNR_RA_v5_VPro_022718(input_filename)
    ...
    % Updated for use in vectrino-data-analysis package by Colin Keating,
    % Spring 2018.  No functional modifications beyond readability improvements at this time.
    ...
    %USER INPUT SECTION
    %filename = '020818_test_results.mat';
    filename = char(input_filename);
    ...
    %to_save = strcat('../processed_files/', filename(1:end-4),'_processed.mat');
    to_save = strsplit(filename, '/'); 
    to_save = to_save{length(to_save)};
    to_save = strcat('../processed_data/', to_save(1:end-4),'_processed.mat');
    ...
    if calc_turbulence == 'y'
        if isProfiler
            ...
            % create matrix to hold all vp*vp data, not just mean
            %% TODO
            vpvp_full = cell(nCells,1);
            ...
            for c = 1:nCells % treat each depth cell independently
                ...
                one_cell_vpvp_full = zeros(length(vp_clean{c}),4,4);
                for ii = 1:4
                    for jj = 1:4
                        vpvp_mean(ii,jj,c) = mean(vp_clean{c}(:,ii).*vp_clean{c}(:,jj),1);
                        one_cell_vpvp_full(:,ii,jj) = vp_clean{c}(:,ii).*vp_clean{c}(:,jj);
                    end
                end
                vpvp_full(c,:) = {one_cell_vpvp_full};
            end
        else % point ADV
            if exist('msg') 
                vpvp_mean = NaN;
    ...
    if ~exist('msg') % don't save any data if we produced an error message
        if ~isempty(t)
            ...
            if calc_turbulence == 'y'
                save(to_save, 'v', 't', 'vpvp_mean', 'pct_rm', 'vpvp_full')
            else
                save(to_save, 'v', 't','pct_rm')
            end
            fprintf('%s%d', 'First valid time (min) is ', single(mint))
        else
            figure, plot(counter, v)
            if calc_turbulence == 'y'
                save(to_save, 'v', 'counter', 'vpvp_mean', 'pct_rm', 'vpvp_full')
            else
